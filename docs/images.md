...
Hybrid mode prefers cached pre-rounded PNGs, falls back to the shader path, and uses a CPU-painted rounded snapshot when Qt Quick is running on its software backend.
...
- `requestRoundedImage(...)` never waits for the SQLite/file-cache worker. A provider-thread ready map serves known variants immediately; misses reserve one coalesced asynchronous lookup/generation job, return an empty string, and emit `roundedImageReady(url, fileUrl)` when an existing or newly generated variant is ready. Ready-entry LRU touches are asynchronous and coalesced.
- Generation triggers after cache hits/misses when preprocessing is enabled; existing variants discovered asynchronously and newly written variants use the same generation-checked signal path. Cache clears invalidate pending lookups, generated bytes, ready-map entries, and late signals together.
...
- One base `Image` feeds both the shader and software fallback, eliminating duplicate image-provider/network requests. The pre-rounded `Image` is instantiated lazily only when that path is eligible and has a source.
- Qt Quick's rectangular `clip` cannot mask children to a `Rectangle.radius`. On the software backend, the lazy `SoftwareRoundedImage` painted item captures the already-loaded base item and draws it through an antialiased `QPainterPath`; the base remains visible until that snapshot is ready. The painted item is not instantiated on the normal GPU path.
...
- Functional: Library grid shows rounded posters; when rounded PNGs are missing, the shader or CPU-painted software fallback renders transparent corners correctly.
