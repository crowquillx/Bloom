...
- Network attempts and credential-refresh waits have a 15-second default deadline, and responses have a 20 MiB limit. Decodes are scaled through `QImageReader` to the requested pixel size when supplied and are rejected above the 192 MiB decoded-image limit. Failed or cancelled jobs discard pending rounded requests; generation-checked writes are ordered against `clearCache()` so pre-clear decode/render work cannot repopulate memory or disk state.
