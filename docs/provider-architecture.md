...
The login screen's `Auto` / `Jellyfin` / `Silo` choice selects Bloom's server protocol adapter; it is not a Silo authentication-provider identifier. Native `/api/v1/auth/login` therefore omits the optional `provider` field so Silo can use its configured/default authentication provider. Authentication completion publishes server/account identity changes before the `authenticated` transition, allowing QML to start catalog requests without those requests being invalidated as stale.
