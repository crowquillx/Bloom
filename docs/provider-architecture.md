...
`track_preferences.json` schema v4 groups season/movie preferences by connection scope and applies the same pending-activation rule. Library SQLite caches and series/movie detail caches use SHA-256 connection-scope directory keys, while static in-memory cache keys include the connection ID. The library memory tier is bounded by an LRU entry/size policy, so switching connections can retain a small reusable snapshot without unbounded growth or cross-connection reuse. `LibraryViewModel` starts a new request generation, reopens its cache, and clears displayed account state when the active connection changes. Logout cancels transport operations and clears library validation, remote-session, and detail-view state without deleting another connection's persisted preferences.
...
`IProviderAuthenticator` owns provider login payloads, response parsing, and validation routes. `JellyfinAuthenticator` implements the existing AuthenticateByName flow while `SiloAuthenticator` validates access/refresh pairs, account identity, profile tokens, and typed error envelopes. Silo profile-PIN success requires `valid: true` and a non-empty `profile_token`; `expires_at` is optional because current native servers can issue tokens scoped to the access-token session without a separate expiry field. `AuthenticationService` remains the stable QML-facing session façade.

The login screen's `Auto` / `Jellyfin` / `Silo` choice selects Bloom's server protocol adapter; it is not a Silo authentication-provider identifier. Native `/api/v1/auth/login` therefore omits the optional `provider` field so Silo can use its configured/default authentication provider. Authentication completion publishes server/account identity changes before the `authenticated` transition. `LibraryService` invalidates catalog work when authentication becomes unavailable, but not while entering `authenticated`, because QML may synchronously issue the first Home request from that same notification.

`HttpTransport` owns the shared `QNetworkAccessManager` and centralizes bounded per-attempt deadlines, cancellation, error mapping, redacted request logging, and unauthorized policy. Retry safety is explicit per operation: requests default to never replaying, idempotent reads may retry, and a caller may opt into replay only when it supplies a stable idempotency mechanism. Retry policies express total attempts, use capped exponential backoff with jitter, honor bounded `Retry-After` values, and cover transport failures plus HTTP 408, 429, 500, 502, 503, and 504. Unauthorized recovery remains single-flight but has its own deadline; a successful recovery may replay the original operation once because the rejected `401` did not apply it. Catalog and remote-session `401` responses expire immediately, while playback reads can defer expiry until playback stops. Canceled work is never retried.

`NetworkError` keeps the compatibility `code` field while separately reporting the Qt network error, HTTP status, provider error code, and a bounded response envelope. Provider adapters should progressively own interpretation of provider-specific error bodies. `SessionService` uses the shared transport instead of a private network manager.

`LibraryService`, `PlaybackService`, and `SessionService` remain stable application façades for catalog/playback/remote-session signals while requests flow through the selected provider adapter, request factory, and transport. Library catalog calls preserve the existing QML methods and legacy signals, while request-aware canonical signals echo caller tokens for view, item, filter, not-modified, and failure outcomes. View models must use those correlated signals whenever one operation owns loading, pagination, refresh, or error state. `SiloCatalogProvider` maps `content_id` catalog identity separately from `file_id` playback/version identity; `SiloArtworkProvider` treats signed URLs as opaque and refetches the owning resource after expiry; `SiloPlaybackProvider` maps the pinned legacy envelope into `PlaybackDescriptor` and provider-native reports. QML must not select protocol routes, construct provider headers, or read credentials. Native Silo protocol v3 remains explicitly unavailable for Bloom/mpv because the pinned capability is `media3_only`, not an mpv contract.
