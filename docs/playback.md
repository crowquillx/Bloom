...
- Embedded libmpv backends (`win-libmpv`, `linux-libmpv-opengl`) apply profile shaders via the base `glsl-shaders` list option with absolute paths resolved from Bloom's mpv config directory (`~~/shaders/...` expands to `{config}/mpv/shaders/...`). CLI-style `--glsl-shaders-append` remains for external mpv only.
