...
- Initial pages, later pages, and SWR refreshes share one connection-scoped cache snapshot. Empty snapshots are valid cache entries. Pagination replaces the requested position range, background refreshes merge refreshed pages with already loaded later pages, and the process-wide memory tier is bounded to 48 datasets or approximately 32 MiB with LRU eviction. Persisted SQLite caches remain isolated by connection scope.
