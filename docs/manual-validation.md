...
Use the exact Silo source revision and immutable image documented in [provider compatibility](provider-compatibility.md): `8044eb84dd0cfa512ce8f2448cfd51cb7899a4c6`. For live checks, use a disposable fixture account and media library. The opt-in watched/favorite probes return the observable boolean state to its baseline, but inverse API operations can still rewrite history, progress, or timestamps; never run them against non-disposable media. Keep the native listener on `8090` and the Jellyfin-compatibility listener on `8096`; never infer native behavior from the compatibility listener. Set `BLOOM_CONTRACT_USERNAME` and `BLOOM_CONTRACT_PASSWORD` in the environment (and `BLOOM_CONTRACT_PROFILE_PIN` only for a PIN profile); never include their values in a command or evidence artifact.
...
| Windows HDR/refresh lifecycle | **NOT RUN / UNAVAILABLE** | On HDR-capable Windows hardware, start SDR, HDR10, and supported Dolby Vision content with refresh matching both enabled and disabled; stop, replace, autoplay, and exit during a switch. | The UI remains responsive; HDR settles before refresh matching and mpv startup; compatible multiples are skipped; exact fractional pacing is retained; replacement/autoplay does not restore then re-switch; stop and exit restore the original HDR/refresh state without stale completions. | Windows HDR-capable runtime hardware is unavailable for this ledger. |
| Linux custom display commands | **NOT RUN / UNAVAILABLE** | Configure real compositor-specific HDR and refresh commands, then repeat start, stop, replacement, autoplay, timeout, and exit flows on the target compositor. | Commands never freeze the UI; placeholders resolve correctly; timeout/cancellation is bounded; HDR-before-refresh ordering and original-state restoration are preserved; command output remains bounded. | A physical Linux display/compositor with safe mode-switch commands is unavailable for this ledger; fake-command integration is automated by `DisplayManagerTest`. |
