...
|---|---|---|---|---|
| Commit and release build | **PASS — Main 2026-08-08** | `git rev-parse HEAD`; `cat VERSION`; `nix build --print-build-logs` | Record the full commit; `VERSION` is exactly `0.8.0`; `nix build` exits 0 and produces the release package without errors. | crowquillx, 2026-08-08: validated commit `99ea18806b77071b265349e1499657e46575c91f`; `VERSION` printed `0.8.0`; `nix build --print-build-logs --option max-jobs 1 --option cores 2` exited 0. |
| Linux offscreen smoke | **PASS — Main 2026-08-08** | `env QT_QPA_PLATFORM=offscreen nix run . -- --version` | Process exits 0, prints Bloom version `0.8.0`, and emits no Qt platform/plugin error. | crowquillx, 2026-08-08, commit `99ea18806b77071b265349e1499657e46575c91f`: command exited 0 and printed `Bloom 0.8.0`. |
| Deterministic test evidence | **PASS — Main 2026-08-08** | `env BLOOM_BUILD_JOBS=2 nix flake check --print-build-logs --option max-jobs 1 --option cores 2` | Command exits 0 and its recorded output shows the deterministic tests and validation derivations completed; record any skipped/unavailable checks rather than treating them as passes. | crowquillx, 2026-08-08, commit `99ea18806b77071b265349e1499657e46575c91f`: `./scripts/dev-build.sh --tests` and local CTest passed 28/28; the exact Nix command exited 0 with provider-contract Python tests 29/29, Nix CTest 26/26, QML lint, package, metadata, and release-manifest checks passing. |
