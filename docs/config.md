...
- `settings.video.skip_refresh_rate_on_compatible_multiple` (Q_PROPERTY `skipRefreshRateOnCompatibleMultiple`): When true, Bloom will skip the refresh rate switch if the current display refresh rate is already an integer multiple of the video framerate (for example 120Hz with 23.976/24fps content). When false, Bloom will always attempt to switch to the exact video framerate. Default: false. Configurable via Settings > Video > Skip switch when refresh rate is already a multiple.
