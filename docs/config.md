...
- `inputBindings` stores keyboard/controller binding overrides under `settings.input_bindings`; defaults live in `InputBindingManager` and omitted actions fall back to defaults. See [docs/input-bindings.md](input-bindings.md).
...
- `settings.connection_state.scopes.<connectionId>.library_profiles` and `.series_profiles` hold per-connection dictionaries keyed by provider library/series IDs. The `_local` scope preserves pre-login/test behavior without mixing it with authenticated connections.
...
- Config v24 adds startup buffering mode settings. `settings.playback.startup_buffering_mode` defaults to `normal`; config v29 moves optional per-library overrides under each `settings.connection_state.scopes.<connectionId>.library_startup_buffering_modes` dictionary. Valid values are `normal` and `remote-mount`; missing library entries use the global value.
