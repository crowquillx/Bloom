...
- `audioOutputDevice` is a Q_PROPERTY string stored at `settings.playback.audio_output_device` (default `auto`). `auto` (or empty) follows the system default output device; a specific value is an mpv audio-device id (e.g. `wasapi/{...}`) passed to mpv via `--audio-device`. See [docs/playback.md](playback.md) for the hotplug/reconnect behavior.
