...
Dialog {
    ...
    property string profileToRename: ""
    property Item restoreFocusTarget: null

    signal profileRenamed(string oldName, string newName)
    ...
    function duplicateName() {
        ...
        return name !== "" && name !== root.profileToRename && ConfigManager.mpvProfileNames.indexOf(name) >= 0
    }
    ...
    footer: Item {
        ...
        RowLayout {
            ...
            Button {
                ...
                Keys.onRightPressed: { if (renameBtn.enabled) renameBtn.forceActiveFocus(); event.accepted = true }
            ...
            Button {
                ...
                enabled: nameField.text.trim() !== "" && nameField.text.trim() !== root.profileToRename && !root.duplicateName()
                Keys.onUpPressed: { nameField.forceActiveFocus(); event.accepted = true }
                ...
                contentItem: Text { text: renameBtn.text; font.pixelSize: Theme.fontSizeBody; font.family: Theme.fontPrimary; color: parent.enabled ? Theme.textPrimary : Theme.textDisabled; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter }
                background: Rectangle { implicitHeight: Theme.buttonHeightSmall; radius: Theme.radiusSmall; color: parent.enabled ? (renameBtn.hovered ? Theme.buttonPrimaryBackgroundHover : Theme.buttonPrimaryBackground) : Theme.buttonSecondaryBackground; border.color: renameBtn.activeFocus ? Theme.focusBorder : Theme.inputBorder; border.width: renameBtn.activeFocus ? 2 : 1 }
            }
        }
    }

    onOpened: {
        nameField.text = root.profileToRename
    ...
    onAccepted: {
        profileRenamed(root.profileToRename, nameField.text.trim())
        profileToRename = ""
        nameField.text = ""
    }
