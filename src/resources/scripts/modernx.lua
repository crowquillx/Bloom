...
-- Bloom segment data for skip intro/outro
local bloom_segments = {
    intro_start = nil,
    intro_end = nil,
    outro_start = nil,
    outro_end = nil,
    current_segment = nil,  -- "intro", "outro", or nil
    osd_overlay = nil,      -- OSD overlay for skip message
    message_visible = false, -- Is the skip message currently showing
    last_segment_shown = nil, -- Track which segment we last showed message for
    message_timer = nil,    -- Timer to auto-hide message after timeout
    suppressed_segment = nil, -- Segment type suppressed until leaving segment
}

-- Skip message timeout in seconds
local skip_message_timeout = 3

-- Forward declarations for skip functions (defined later)
local show_skip_message, hide_skip_message, enable_skip_bindings, disable_skip_bindings
...
-- Bloom segment handlers
mp.register_script_message("bloom-segment-intro", function(start_time, end_time)
    bloom_segments.intro_start = tonumber(start_time)
    bloom_segments.intro_end = tonumber(end_time)
    msg.info("Received intro segment: " .. start_time .. " - " .. end_time)
end)

mp.register_script_message("bloom-segment-outro", function(start_time, end_time)
    bloom_segments.outro_start = tonumber(start_time)
    bloom_segments.outro_end = tonumber(end_time)
    msg.info("Received outro segment: " .. start_time .. " - " .. end_time)
end)

mp.register_script_message("bloom-clear-segments", function()
    bloom_segments.intro_start = nil
    bloom_segments.intro_end = nil
    bloom_segments.outro_start = nil
    bloom_segments.outro_end = nil
    bloom_segments.current_segment = nil
    bloom_segments.last_segment_shown = nil
    bloom_segments.suppressed_segment = nil
    hide_skip_message()
    disable_skip_bindings()
    msg.info("Cleared segment data")
end)

-- Show skip message overlay in top-left corner
show_skip_message = function(segment_type)
    if bloom_segments.suppressed_segment == segment_type then
        return -- Suppressed for this segment until we exit it
    end

    if bloom_segments.message_visible and bloom_segments.last_segment_shown == segment_type then
        return -- Already showing this message
    end
    
    -- Cancel any existing timer
    if bloom_segments.message_timer then
        bloom_segments.message_timer:kill()
        bloom_segments.message_timer = nil
    end
    
    -- Create overlay if needed
    if not bloom_segments.osd_overlay then
        bloom_segments.osd_overlay = mp.create_osd_overlay("ass-events")
    end
    
    local message
    if segment_type == "intro" then
        message = "Press → to skip intro"
    else
        message = "Press → to skip credits"
    end
    
    -- Position in top-left with some padding, using ASS formatting
    local ass = assdraw.ass_new()
    local osd_w, osd_h = mp.get_osd_size()
    
    -- Style: semi-transparent background box with white text
    ass:new_event()
    ass:pos(30, 30)
    ass:an(7) -- Top-left alignment
    ass:append("{\\fs24\\bord2\\shad1\\1c&HFFFFFF&\\3c&H000000&\\4c&H000000&\\alpha&H00&}")
    ass:append(message)
    
    bloom_segments.osd_overlay.data = ass.text
    bloom_segments.osd_overlay:update()
    bloom_segments.message_visible = true
    bloom_segments.last_segment_shown = segment_type
    
    -- Start timeout timer to auto-hide the message
    bloom_segments.message_timer = mp.add_timeout(skip_message_timeout, function()
        msg.info("Skip message timeout - hiding")
        bloom_segments.suppressed_segment = segment_type
        hide_skip_message()
        disable_skip_bindings()
        bloom_segments.message_timer = nil
    end)
    
    msg.info("Showing skip message for: " .. segment_type)
end

-- Hide skip message overlay
hide_skip_message = function()
    if bloom_segments.osd_overlay then
        bloom_segments.osd_overlay:remove()
    end
    bloom_segments.message_visible = false
    bloom_segments.last_segment_shown = nil
...
-- Perform the actual skip to segment end
local function do_segment_skip()
    if bloom_segments.current_segment == "intro" and bloom_segments.intro_end then
        msg.info("Skipping intro to: " .. bloom_segments.intro_end)
        mp.commandv("seek", bloom_segments.intro_end, "absolute")
        hide_skip_message()
        return true
    elseif bloom_segments.current_segment == "outro" and bloom_segments.outro_end then
        msg.info("Skipping outro to: " .. bloom_segments.outro_end)
        mp.commandv("seek", bloom_segments.outro_end, "absolute")
        hide_skip_message()
        return true
    end
    return false
...
-- Intercept seek forward commands when skip message is visible
local original_seek_forward = nil
local function handle_seek_forward()
    if bloom_segments.message_visible and bloom_segments.current_segment then
        -- User is seeking forward while in a segment with message visible - skip to end
        if do_segment_skip() then
            return -- Skip was performed, don't do normal seek
        end
    end
    -- Otherwise, let normal seek happen (we don't block it)
end

-- Track if skip bindings are currently active
local skip_bindings_active = false

-- Forward declarations for key binding functions
local on_seek_forward_key, on_seek_forward_large_key, on_l_key

-- Enable skip key bindings (called when entering a segment)
enable_skip_bindings = function()
    if skip_bindings_active then return end
    mp.add_forced_key_binding("RIGHT", "bloom-skip-right", on_seek_forward_key)
    mp.add_forced_key_binding("Shift+RIGHT", "bloom-skip-shift-right", on_seek_forward_large_key)
    mp.add_forced_key_binding("l", "bloom-skip-l", on_l_key)
    skip_bindings_active = true
    msg.info("Skip key bindings enabled")
end

-- Disable skip key bindings (called when leaving a segment)
disable_skip_bindings = function()
    if not skip_bindings_active then return end
    mp.remove_key_binding("bloom-skip-right")
    mp.remove_key_binding("bloom-skip-shift-right")
    mp.remove_key_binding("bloom-skip-l")
    skip_bindings_active = false
    msg.info("Skip key bindings disabled")
end

-- Update which segment we're in based on current position
local function check_segment_position()
    ...
    local prev_segment = bloom_segments.current_segment
    
    -- Check if in intro segment
    if bloom_segments.intro_start and bloom_segments.intro_end then
        if pos >= bloom_segments.intro_start and pos < bloom_segments.intro_end then
            bloom_segments.current_segment = "intro"
            if bloom_segments.suppressed_segment == "intro" then
                return
            end
            show_skip_message("intro")
            enable_skip_bindings()
            return
        end
    end
    
    -- Check if in outro segment
    if bloom_segments.outro_start and bloom_segments.outro_end then
        if pos >= bloom_segments.outro_start and pos < bloom_segments.outro_end then
            bloom_segments.current_segment = "outro"
            if bloom_segments.suppressed_segment == "outro" then
                return
            end
            show_skip_message("outro")
            enable_skip_bindings()
            return
        end
    end
    ...
    -- Not in any segment
    if bloom_segments.current_segment ~= nil then
        bloom_segments.current_segment = nil
        hide_skip_message()
        disable_skip_bindings()
        bloom_segments.suppressed_segment = nil
...
-- Watch time-pos to update segment state
mp.observe_property("time-pos", "number", function(name, value)
    check_segment_position()
end)

-- Define the key binding handler functions
-- RIGHT arrow - typically seeks forward
on_seek_forward_key = function()
    if bloom_segments.message_visible and bloom_segments.current_segment then
        if do_segment_skip() then
            return -- Skip performed
        end
    end
...
-- Shift+RIGHT - larger seek forward
on_seek_forward_large_key = function()
    if bloom_segments.message_visible and bloom_segments.current_segment then
        if do_segment_skip() then
            return
        end
...
-- l key - common seek forward key in some configs
on_l_key = function()
    if bloom_segments.message_visible and bloom_segments.current_segment then
        if do_segment_skip() then
            return
...
-- Register script message for external skip requests
mp.register_script_message("bloom-try-skip-segment", function()
    if bloom_segments.message_visible and bloom_segments.current_segment then
        do_segment_skip()
    end
end)
